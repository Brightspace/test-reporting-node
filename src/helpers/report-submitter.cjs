...
// const repoSettingsDocUrl = 'https://github.com/Brightspace/repo-settings/blob/main/docs/test-reporting.md#analytics'; // TODO Still needed?

// TODO
// - Issue this is a public repo... Anything a concern?
...
/**
 * AWS credentials must be set as environment variable. These credentials must be for a role that can write to Timestream
 */
class ReportSubmitter {
	...
	constructor(logger) {
		...
		this.#experienceMaps = new WeakMap();
	...
	async submit(report) {
		...
		if (this.#experienceMaps.has(report)) {
	...
	#applyExperienceDimensions(report, detailWriteRequests) {
		const map = this.#experienceMaps.get(report);

		if (!map) {
			return;
		}

		this.#experienceMaps.delete(report);
	...
	async #writeTimestream(credentials, requests) {
		const client = new TimestreamWriteClient({ credentials, region: this.region });
